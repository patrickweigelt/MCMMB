...
grid <- readOGR("data/30min_grid_select50/30min_grid_select50%.shp", integer64="allow.loss")
...
americas <- readOGR("data/americas/americas.shp") # add shape file showing the borders of the Neotropics  
...
# add pseudo-absences
set.seed(100)
geo_pau_abs <- rbind(grid_centroids[sample(nrow(grid_centroids),nrow(geo_pau), replace = TRUE),],geo_pau)
...
# add pseudo-absences
set.seed(100)
geo_und_abs <- rbind(grid_centroids[sample(nrow(grid_centroids),nrow(geo_und), replace = TRUE),],geo_und)
...
# add pseudo-absences
set.seed(100)
phy_ten_abs <- rbind(grid_centroids[sample(nrow(grid_centroids),nrow(phy_ten), replace = TRUE),],phy_ten)
...
# add pseudo-absences
set.seed(100)
mau_fle_abs <- rbind(grid_centroids[sample(nrow(grid_centroids),nrow(mau_fle), replace = TRUE),],mau_fle)
...
geo_pau_abs <- geo_pau_abs[complete.cases(geo_pau_abs@data[,c("elev","bio1","bio12")]),]
...
geo_und_abs <- geo_und_abs[complete.cases(geo_und_abs@data[,c("elev","bio1","bio12")]),]
...
phy_ten_abs <- phy_ten_abs[complete.cases(phy_ten_abs@data[,c("elev","bio1","bio12")]),]
...
mau_fle_abs <- mau_fle_abs[complete.cases(mau_fle_abs@data[,c("elev","bio1","bio12")]),]
```
&nbsp;
