...
We also import the georeferenced grid and the outline of Americas.
...
Among the traits available, we have different trait types, related to growth
form and habit, armature, stem size, leaf and fruit.

### 1.2.1. Trait type

Traits can be either binary traits (0 or 1, sometimes 2 when one species varies
in that trait), categorical or continuous.

```{r}
str(tra)
```

To get the frequency of caterogies of categorical and binary traits, we can
...
For sake of simplicity, we will only use continuous traits.  
