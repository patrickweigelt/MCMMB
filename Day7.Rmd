---
title: 'Day 7: Phylogenetic diversity'
...
**Preparation:** Please open your RStudio project and download the data for today ('Data_Day7.zip') from [Stud-IP](https://www.studip.uni-goettingen.de/) and copy it into your .Rproj folder '/Data/' (or '/data/). You can use `getwd()` to locate your current working directory, which should be your project folder. Please install the following R-packages using `install.packages()`:  
...
library(PhyloMeasures) # Phylogenies and ecology
...
_Add a column for the full species name with a hyphen as separator: "species"_  
```{r,echo=FALSE}
...
where mean.null is the mean of the null values and sd.null is the standard deviation of the null values.  
...
pd.null$new_ID<-rownames(pd.null)

pd.out    <- pd.null[,c(21, 1,13,2,14,3,15,4,16)]  # select columns for final data set
pd.out [is.na(pd.out )] <- 0  # change NAs to zeroes; no phylogenetic diversity if grid has only 1 spp!


par(mfrow=c(2,1))
plot(pd.out$Faith_PD.observed, pd.out$Corrected_FaithPD.observed, main=" Faith's PD vs Faith's corrected PD", xlab="observed Faith's PD",ylab="corrected Faith's PD", cex.main=0.8,cex.axis=0.8)
...
**Tasks**:  
1. Calculate species richness per 'new' gridcell.    
2. Create a data.frame called 'palmdiversity' with species richness and phyogenetic diversity metrics.    
3. Plot correlations among the diversity metrics.   
4. Join the 'palmdiversity' data.frame to the grid_2degrees shapefile to be able to link the diversity metrics and the poylgons for plotting.  
5. Plot maps with colours according to the diversity metrics.  

**Exercise II solutions:**

_Calculate species richness per "new" gridcell_
```{r, echo=FALSE}
richness <- rowSums(species_grid)
richness[1:10]
```
&nbsp;

_Create a data.frame called 'palmdiversity' by joining richness with 'pd.out'_

```{r, echo=FALSE}
palmdiversity<- data.frame(new_ID = as.numeric(rownames(species_grid)), richness) 
pd.out$new_ID<-as.numeric(pd.out$new_ID)
...
pairs.panels(palmdiversity[,c(2:10)], method="pearson",density=F,ellipses=F, hist.col = "white")
```
&nbsp;
...
*Species richness, MPD, \& MNTD (absolute)*  
```{r echo=FALSE, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
par(mfrow=c(1,3),mar=c(1,1,1,1), oma=c(0,0,0,0))

my.class.fr<-classIntervals(grid_2degrees@data$richness,n=10,style="equal") # bin data into n quantiles
...
*Species richness, MPD, \& MNTD (standardized)*  
