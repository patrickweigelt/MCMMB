...
# 0. Before starting
In R, there are two major ways to plot your data: **base R** (as we did so far) and **ggplot2**.  
There is no better way to plot things, it is rather a matter of taste, but it is quite important to know how both work.  
Let's see with an easy example how both work.  
...
cols <- viridis(n = n_distinct(grid$spec_num))
# pal <- colorRampPalette(cols)

colours <- viridis(n = 10)
colours <- colorRamp(colours)(grid$spec_num/max(grid$spec_num))
...
legend("bottomleft", # position
       legend = c(round(range(grid$spec_num), 1)),
